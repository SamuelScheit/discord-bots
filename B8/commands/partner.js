module.exports.init = function() {
	setInterval(() => {
		config.partner.forEach(p => {
			if (new Date().getTime() - p.last > 172800000) {
				client.guilds
					.first()
					.channels.get("567681165448970290")
					.send({
						embed: {
							author: {
	...
	client.on("messageReactionAdd", (reaction, user) => {
		...
		switch (reaction.emoji.name) {
			case "❌":
				member.send(
					"Die Anforderungen für eine Partnerschaft haben leider nicht gereicht!\nVersuche es bitte erst in einem Monat erneut!\nHier sind die Anforderungen: <#568136877811433472>"
				);
				user.send("Erfolgreich abgelehnt");
				break;
			case "✅":
				member.send(
					"Du bist nun akzeptiert mit deinem Discord, bitte gib <@457947671072604170> die Partner Rolle, damit du bei uns Partner bist, schreib in <#560178606291288074> mit ```!partner text [Text]``` deinen Text, er wird dann 3x die Woche gepostet."
				);
				member.addRole("567681143902830602");
...
module.exports.run = function(message, args) {
	switch (args[0]) {
		case "add":
			if (message.channel.id != "568136877811433472")
				return message.channel
					.send({
						embed: {
							...
							description:
								"Du kannst diesen Befehl nur in <#568136877811433472> ausführen!"
						}
					})
					.then(msg => {
			...
			if (args[1] == undefined) {
				...
				message.member.addRole("569506988652888066");
				invite = args[1];

				message.guild.roles
					.filter(x => x.id == "567681135434268682")
					.forEach(role => {
			...
			break;
		case "text":
			if (message.channel.id != "569506040811356192")
				return message.channel
					.send({
						embed: {
							color: 0x808080,
							title: "Partner: Fehler",
							description:
								"Du kannst diesen Befehl nur in <#569506040811356192> ausführen!"
						}
					})
					.then(msg => {
						setTimeout(m => m.delete(), 10000, msg);
		...
		case "remove":
			...
			try {
				if (
					message.member.roles.find(x => x.id == "567681135434268682")
				) {
					message.channel
						.send({
							embed: {
